...
def generate_sb3_json(frames_json, sound_json, FPS, size):
    ...
    sb3_json["targets"][1]["x"] = -240
    sb3_json["targets"][1]["y"] = 180
    sb3_json["targets"][1]["blocks"]["affpKuZ1)zSJ}%ISFpzJ"]["fields"]["SOUND_MENU"] = [sound_json["name"], None]
    return sb3_json

def generate_costume_json(frame_path):
    return {
        "name":"",
...
def generate_sb3_file(video_path, output_path):
    count = 0
    ...
    while success:
        ...
        success, image = video_capture.read()
        count += 1
        costume_json.append(generate_costume_json(f"{md5_hash}.png"))
    ...
    with open(f"{temp_dir}/project.json", "w", encoding="utf-8") as file:
        ...
        json.dump(generate_sb3_json(costume_json, generate_sound_json(sound_path, rate, sample_count), video_clip.fps, ((480/width)+(1/3))*100), file)
        audio_clip.close()
        video_clip.close()
...
generate_sb3_file(sys.argv[1], output_path)
remove_temp_dir()
